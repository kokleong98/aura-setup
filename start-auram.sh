...
# VERSION=0.1.0 
# FILENAME=start-auram.sh
# DESCRIPTION=AURA-M Systemd main routine script
####################################################################
...
if [ -f "${DIR}/auram.conf" ]; then
  source "${DIR}/auram.conf"
  echo "Loading auram.conf settings."
fi
  ...
  do
    ...
    if [ $? -eq 0 ]; then
      ...
      if [ ! -z "$processingblock" ]; then
        ...
        if [[ $((blocknum - processingblock)) -lt 10 ]]; then
          break
        fi
      fi
    fi
